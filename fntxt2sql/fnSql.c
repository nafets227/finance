...
const char * const * getCreateSql(void)
{
    ...
    static const char achCreViewBalance[] = "CREATE OR REPLACE VIEW %s_balance AS "
        "SELECT "
            "ORIG_KTONR, ORIG_BLZ, JAHR, MONAT, "
        ...
        "FROM ( "
            "SELECT DISTINCT "
                ...
                "last_value(datum) OVER ( "
                    "partition by orig_ktonr, orig_blz, jahr, monat "
                ...
                "FROM ( "
                    "SELECT "
                        ...
                        "sum(if(buchart='E',1,0)) OVER ( "
                            "partition by orig_ktonr, orig_blz "
                            "order by orig_ktonr, datum, buchart desc "
                            ") as saldo_partition "
                    "FROM %s "
    ...
    static const char achCreViewTodo[] = "CREATE OR REPLACE VIEW %s_todo AS "
        "SELECT KATG5, KATG5_BETRAG, "
            ...
            "KATG2, KATG2_BETRAG, "
            "DATUM_KOR, KATG, "
            "ORIG_KTONR, VALUTA, BETRAG, PART_NAME1, PART_NAME2, BUTEXT, "
            "VZWECK1, VZWECK2, VZWECK3, VZWECK4, VZWECK5, VZWECK6, VZWECK7, "
            "ID "
        "FROM fn_entry "
